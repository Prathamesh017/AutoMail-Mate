...
const EmailPop = ({ isOpen, setIsOpen, content,sender }) => {
  ...
  async function replyEmail(){
    ...
    let endpoint=mailType === 'gmail' ? 'gmail' : 'outlook'
      ...
      await axios.post(`${url}/mail/${endpoint}/reply`,{
          From:userEmail,
          To:sender,
          content:text,
        },
  ...
  async function generateResponse() {
      try {
        const response = await axios.post(`${url}/mail/response`,{
          content
        });
      ...
      >
        <div className="bg-white rounded-lg p-6 shadow-lg w-3/4 md:w-1/4 ">
          ...
          <div className="flex justify-end">
            <button
              onClick={replyEmail}
              className="bg-sky-700 p-2 rounded hover:text-slate-200"
            >
              Reply Email
            </button>
          </div>
          {isSuccess && (
            <div>
              <p className="text-green-800 text-center mt-4 text-xl">
